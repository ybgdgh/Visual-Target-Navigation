...
class VectorEnv:
    ...
    def _worker_env(
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                ...
                elif command == GET_LOCAL_ACTIONS:
                    connection_write_fn(env.get_local_actions(data))
    ...
    def get_local_actions(self, data: List[Union[int, str, Dict[str, Any]]]) -> List[Any]:
        self.async_get_local_actions(data)
        return self.wait_get_local_actions()

    def async_get_local_actions(self, data: List[Union[int, str, Dict[str, Any]]]) -> None:

        if isinstance(data[0], (int, np.integer, str)):
            data = [{"goal": {"goal": goal}} for goal in data]

        self._is_waiting = True
        for write_fn, args in zip(self._connection_write_fns, data):
            write_fn((GET_LOCAL_ACTIONS, args))

    def wait_get_local_actions(self) -> List[Observations]:
