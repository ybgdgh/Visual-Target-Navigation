...
class GlobalRolloutStorage:
    ...
    def __init__(
        ...
        for sensor in g_observation_space.spaces:
            self.observations[sensor] = torch.zeros(
                num_steps + 1,
                num_envs,
                *g_observation_space.spaces[sensor].shape
            )
        ...
        if g_action_space.__class__.__name__ == "ActionSpace":
            g_action_shape = 1
        else:
            g_action_shape = g_action_space.shape[0]

        self.g_actions = torch.zeros(num_steps, num_envs, g_action_shape)
        self.prev_g_actions = torch.zeros(num_steps + 1, num_envs, g_action_shape)
        if g_action_space.__class__.__name__ == "ActionSpace":
            self.g_actions = self.g_actions.long()
            self.prev_g_actions = self.prev_g_actions.long()
    ...
    def to(self, device):
        ...
        self.g_actions = self.g_actions.to(device)
        self.prev_g_actions = self.prev_g_actions.to(device)
    ...
    def insert(
        ...
        self.g_actions[self.step].copy_(g_actions)
        self.prev_g_actions[self.step + 1].copy_(g_actions)
    ...
    def after_update(self):
        ...
        self.prev_g_actions[0].copy_(self.prev_g_actions[self.step])
        self.step = 0
    ...
    def recurrent_generator(self, advantages, num_mini_batch):
        ...
        # print(num_envs_per_batch, num_processes)
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                # print("ind: ", ind)
                # print("self.g_actions: ", self.g_actions.shape) # 64 * 2 * 2
                actions_batch.append(self.g_actions[: self.step, ind])
                # print("g_actions: ", self.g_actions[: self.step, ind].shape) # 64*2

                prev_actions_batch.append(self.prev_g_actions[: self.step, ind])
